...
fun senha(listaQuartos: MutableList<Boolean>, dadosRelatorio : RelatorioDados) {
    ...
    while (tentavias > 0) {
        ...
        if (tentavias > 0) {
            ...
            println("Tentativas restantes: $tentavias")
        }
    }
    if (tentavias == 0) {
...
fun cadastrarHospede(listaHospedes: MutableList<String>, dadosRelatorio : RelatorioDados) {
    ...
    println("Olá $usuario, bem-vindo(a) ao cadastro hospedes!")
    println()
    print("""
    1- Cadastro de Hospedes
    2- Pesquisar Hospede por nome exato
    3- Pesquisar Hospede por prefixo
    4- Listar Hospedes ordenadamente (A/Z)
    ...
    when (escolha) {
        1 -> {
            ...
            if (listaHospedes.size >= limiteDeHospedes) {
                println("Limite de $limiteDeHospedes hospedes atingidos! Fale com o gerente para liberar mais hospedes!")
                return
            }
            ...
            if(listaHospedes.isEmpty()){
             println("Nenhum Hóspede para ter seu cadastro atualizado!")
                ...
                if (escolha == null || escolha < 1 || escolha > listaHospedes.size+1) {
                    println("Opção invalida! Operação cancelada!")
                    ...
                    println("Digite a atualização do Hópede: ")
        ...
        6 ->{
            if(listaHospedes.isEmpty()){
                ...
                println("Selecione o número do Hópede que deseja remover: ")
                listaHospedes.forEachIndexed { index, hospede ->
                    println("[${index + 1}] $hospede ")
                }
                println("[${listaHospedes.size + 1}] Cancelar e voltar ao painel!]")
                ...
                if (escolha == null || escolha < 1 || escolha > listaHospedes.size+1) {
                    println("Operação invalida! Operação cancelada!")
                }else if (escolha == listaHospedes.size+1) {
                    println("Operação cancelada!")
                }else{
                    val numeroDoHospede = escolha - 1
...
fun quartos(listaQuartos: MutableList<Boolean>, dadosRelatorio: RelatorioDados) {
    ...
    println()
    println("Informe o valor da diaria: ")
    val valorDiaria = readln().toDoubleOrNull()
    println("Iforme a quantidade de diarias (1-30): ")
    ...
    if (valorDiaria == null ||
        ...
        println("Valor invalido, retornando ao inicio da reserva!")
        println()
        return(quartos(listaQuartos, dadosRelatorio))
    }
    println("Informe o nome do hospede que está fazendo a reserva: ")
    val hospede = readln()
    ...
    val tipoQaurtoSEL = when (tipoDoQuarto) {
        ...
        "L" -> 1.65
            ...
            println("Tipo de quarto ivalido, retornando ao inicio da reserva!")
            println()
            return(quartos(listaQuartos, dadosRelatorio))
        }
    }
    ...
    while (true) {
        ...
        if (numeroDoQuarto == null || numeroDoQuarto <= 0 || numeroDoQuarto > 20) {
            println("Numero do quarto invalido, voltando as reservas!")
            println()
            return(quartos(listaQuartos, dadosRelatorio))
        }
    ...
    println("Hospede que fez a reserva: $hospede")
    println("Quarto reservado: $numeroDoQuarto, tipo do quarto: $tipoDoQuarto")
    println("Diarias reservadas: $diaria")
...
fun eventos(dadosRelatorio: RelatorioDados) {
    ...
    while (true) {
        println("Iforme a quantidade de convidados:")
    ...
    while (true) {
        ...
        if (duracaoDigitada == null || duracaoDigitada !in 1..12) {
            println("Duração invalida! Deve ser entre 1 e 12 horas!")
            continue
        }
        duracao = duracaoDigitada

        val limiteEvento = if (diaSemana == "sabado" || diaSemana == "domingo" || diaSemana == "sábado") 15 else 23
        val horaTermino = horaInicial + duracao

        if (horaInicial < 7 || horaTermino > limiteEvento) {
            println("Auditorio indisponivel de acordo com as regras!")
    ...
    println("Auditorio selecionado: $auditorioSelecionado")
